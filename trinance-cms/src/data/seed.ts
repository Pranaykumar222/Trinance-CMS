...
const AVATAR_COLORS = ["#4f46e5", "#0ea5e9", "#16a34a", "#f59e0b", "#dc2626", "#9333ea", "#0891b2", "#db2777"];
...
function makeSubscriber(i: number): Subscriber {
  const first = pick(FIRST);
  const last = pick(LAST);
  const name = `${first} ${last}`;
  const planRoll = rand();
  const plan: Subscriber["plan"] =
    planRoll < 0.32 ? "free" : planRoll < 0.55 ? "monthly" : planRoll < 0.78 ? "quarterly" : "yearly";
  ...
  const payments = Array.from({ length: cycles }).map((_, p) => ({
    ...
    status: (rand() < 0.94 ? "paid" : rand() < 0.6 ? "failed" : "refunded") as "paid" | "failed" | "refunded",
    method: pick(METHODS),
  }));
...
export const subscribers: Subscriber[] = Array.from({ length: 54 }).map((_, i) => makeSubscriber(i + 1));
...
/* ------------------------------------------------------------------ */
/* Time-series analytics (deterministic)                               */
/* ------------------------------------------------------------------ */
export function buildTimeSeries(days: number) {
  const r = mulberry32(days + 7);
  let subs = 3600;
  let rev = 42000;
  const out: { date: string; subscribers: number; revenue: number; opens: number; clicks: number }[] = [];
  for (let i = days; i >= 0; i--) {
    subs += Math.floor(r() * 40) + 6;
    rev += Math.floor(r() * 900) + 120;
    const opens = 8000 + Math.floor(r() * 6000);
    out.push({
      date: iso(NOW - i * DAY),
      subscribers: subs,
      revenue: rev,
      opens,
      clicks: Math.floor(opens * (0.16 + r() * 0.1)),
    });
  }
  return out;
}
